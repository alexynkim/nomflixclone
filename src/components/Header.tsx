...
const Item = styled.li<{ $isbgblack: boolean }>`
  margin-right: 20px;
...
const Input = styled(motion.input)<{ $isbgblack: boolean }>`
  ...
  padding-left: 40px;
  z-index: -1;
  color: ${(props) => (props.$isbgblack ? "white" : "black")};
  font-size: 16px;
  ...
  useEffect(() => {
    scrollY.on("change", () => {
      if (scrollY.get() > 80) {
        ...
        navAnimation.start("top").then(() => setScrollUpdate(scrollY.get()));
      }
    });
      ...
      <Col>
        <Search onSubmit={handleSubmit(onValid)}>
          <motion.svg
            ...
            fill={scrollUpdate > 80 ? "white" : "black"} //"currentColor"
            viewBox="0 0 20 20"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
